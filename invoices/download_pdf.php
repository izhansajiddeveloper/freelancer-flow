...
// Save to DB and download
$filename = 'Invoice_' . str_replace('-', '', $inv['invoice_number']) . '_' . time() . '.pdf';
$filepath = '../uploads/invoices/' . $filename;

// Ensure directory exists
if (!is_dir('../uploads/invoices/')) {
    mkdir('../uploads/invoices/', 0777, true);
}

// Save File locally to DB
$pdf->Output(dirname(__DIR__) . '/uploads/invoices/' . $filename, 'F');

// Update DB record
$stmt = $pdo->prepare("UPDATE invoices SET pdf_file = ? WHERE id = ?");
