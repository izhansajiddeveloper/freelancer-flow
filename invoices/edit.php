...
// Only drafts can be fully edited generally, but we'll allow it anyway or show a warning
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($client_id) || empty($invoice_number) || empty($issue_date) || empty($total_amount)) {
        ...
        try {
            $stmt = $pdo->prepare("
                UPDATE invoices 
                SET client_id = ?, project_id = ?, invoice_number = ?, issue_date = ?, due_date = ?, subtotal = ?, tax = ?, total_amount = ?, status = ?, notes = ?
    ...
    <main class="main-content">
        <div class="dashboard-topbar">
            <div class="topbar-left">
                <h2 style="font-weight: 800; letter-spacing: -0.5px;">Edit Invoice</h2>
            </div>
            <div class="topbar-actions">
                <a href="view.php?id=<?php echo $id; ?>" class="btn btn-outline" style="border-radius: 12px;"><i class="fas fa-eye"></i> View Original</a>
            </div>
        </div>
                    ...
                    <form method="POST">
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-bottom: 25px;">
                            ...
                            <div class="form-group">
                                ...
                                <select id="client_selector" name="client_id" style="width: 100%; padding: 14px 20px; border: 1px solid #e2e8f0; border-radius: 12px; outline: none; background: white;" required>
                                    ...
                                    <?php foreach ($clients as $c): ?>
                                        <option value="<?php echo $c['id']; ?>" <?php echo $c['id'] == $invoice['client_id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($c['client_name']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select id="project_selector" name="project_id" style="width: 100%; padding: 14px 20px; border: 1px solid #e2e8f0; border-radius: 12px; outline: none; background: white;">
                                    <option value="">-- Select Project --</option>
                                    <?php 
                                    foreach ($projects_data as $p) {
                                        if ($p['client_id'] == $invoice['client_id']) {
                                            $sel = ($p['id'] == $invoice['project_id']) ? 'selected' : '';
                                            echo "<option value=\"{$p['id']}\" $sel>" . htmlspecialchars($p['project_title']) . "</option>";
                                        }
                                    }
                                    ?>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="invoice_number" value="<?php echo htmlspecialchars($invoice['invoice_number']); ?>" style="width: 100%; padding: 14px 20px; border: 1px solid #e2e8f0; border-radius: 12px; outline: none; font-family: monospace;" required <?php echo ($invoice['status']!=='draft')?'readonly':''; ?>>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" name="issue_date" value="<?php echo $invoice['issue_date']; ?>" style="width: 100%; padding: 14px 20px; border: 1px solid #e2e8f0; border-radius: 12px; outline: none;" required <?php echo ($invoice['status']!=='draft')?'readonly':''; ?>>
                            </div>

                            <div class="form-group">
                                <label style="display: block; font-weight: 700; margin-bottom: 10px; color: #1e293b;">Due Date</label>
                                <input type="date" name="due_date" value="<?php echo $invoice['due_date']; ?>" style="width: 100%; padding: 14px 20px; border: 1px solid #e2e8f0; border-radius: 12px; outline: none;" required <?php echo ($invoice['status']!=='draft')?'readonly':''; ?>>
                            </div>
                        </div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; margin-bottom: 30px;">
                            <div class="form-group">
                                ...
                                <div style="display: flex; align-items: center; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; background: white;">
                                    ...
                                    <input type="number" step="0.01" id="subtotal" name="subtotal" value="<?php echo $invoice['subtotal']; ?>" style="width: 100%; padding: 14px 20px; border: none; outline: none; font-weight: 600;" required <?php echo ($invoice['status']!=='draft')?'readonly':''; ?>>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div style="display: flex; align-items: center; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; background: white;">
                                    <input type="number" step="0.01" id="tax" name="tax" value="<?php echo $invoice['tax']; ?>" style="width: 100%; padding: 14px 20px; border: none; outline: none; font-weight: 600;" required <?php echo ($invoice['status']!=='draft')?'readonly':''; ?>>
                            ...
                            <div class="form-group">
                                ...
                                <div style="display: flex; align-items: center; border: 2px solid var(--primary-color); border-radius: 12px; overflow: hidden; background: white; box-shadow: 0 4px 10px rgba(79, 70, 229, 0.1);">
                                    ...
                                    <input type="number" step="0.01" id="total_amount" name="total_amount" value="<?php echo $invoice['total_amount']; ?>" style="width: 100%; padding: 14px 20px; border: none; outline: none; font-weight: 800; color: #0f172a; font-size: 1.1rem;" readonly>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group" style="margin-bottom: 30px;">
                            ...
                            <textarea name="notes" rows="6" style="width: 100%; padding: 16px 20px; border: 1px solid #e2e8f0; border-radius: 16px; outline: none; background: #f8fafc; resize: vertical;" placeholder="Thank you for your business! Payment is due within 30 days..."><?php echo htmlspecialchars($invoice['notes']); ?></textarea>
                        </div>
                        ...
                        <div style="display: flex; gap: 15px; margin-top: 40px;">
                            <button type="submit" class="btn btn-primary" style="flex: 2; padding: 16px; border-radius: 16px; font-weight: 800; background: var(--gradient-primary); border: none; font-size: 1.1rem; box-shadow: 0 4px 15px rgba(79, 70, 229, 0.3);">
                                <i class="fas fa-save" style="margin-right: 8px;"></i> Update Title
                            </button>
...
<script>
    ...
    function calculateTotal() {
        if(subtotalInput.readOnly || subtotalInput.disabled) return;
